...
description: 【作業開始時と完了時に必ず実行】tmuxウィンドウ名を開始直後に設定し、完了時はsay報告と再設定を行うSkill。実装・修正・調査などあらゆるタスクで使うこと。
allowed-tools: Bash
---
...
1. ユーザー依頼を受けて方針が固まったら、探索や実装を深く始める前に `rename_tmux_window.sh` を実行する。
2. 引数にはそのタスクを表す**日本語漢字2-6文字程度**の短い概要を渡す。
3. 形式は `<リポジトリ名>-<タスク概要>` を維持する。進捗率、記号、角括弧は付けない。
4. 当初の要約が途中で大きくズレた場合のみ、作業中に1回だけ更新してよい。
5. 雑多な確認だけで終わる短い依頼でも、tmux内で作業しているなら開始時に一度は設定する。
...
2. 作業完了時に、このスキルと同じディレクトリにある `rename_tmux_window.sh` を再度実行する。
3. **引数にタスク概要を日本語漢字2-6文字程度で渡す**。スクリプトが `<リポジトリ名>-<タスク概要>` 形式でウィンドウ名を設定する。
